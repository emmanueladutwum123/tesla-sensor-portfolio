...
class TeslaStyleDetector:
    ...
    def process_frame(self, frame: np.ndarray):
        """Run inference on a single RGB frame. Returns a pandas DataFrame of Tesla-relevant detections.
        """
    ...
    def benchmark_latency(
        ...
        if stats["p99_ms"] < target_ms:
            ...
            print(f"  ✗ mean {stats['mean_ms']:.1f}ms > {target_ms:.0f}ms — optimization required")
    ...
    def visualize_detections(
        ...
        for _, row in relevant.iterrows():
            ...
            print(
                f"  {name:14s}  conf={row['confidence']:.2f}  "
    ...
    def detection_summary(self, frame: np.ndarray) -> dict:
        """Return a structured summary of detections in one frame. Useful for the fusion pipeline.
        """

...
class LiDARDetection:
    ...
    class_id: int
    confidence: float
    centroid: np.ndarray  # [X, Y, Z] in LiDAR frame (meters)
    num_points: int = 0
    timestamp_ms: float = 0.0
...
class FusedDetection:
    ...
    bbox: np.ndarray | None = None  # from camera, if matched
    centroid: np.ndarray | None = None  # from LiDAR, if matched
...
class CameraLiDARCalibration:
    """Holds the extrinsic and intrinsic matrices needed to project a LiDAR point into the image plane.

    For real deployment load these from the vehicle calibration file. The defaults here are plausible for a
    forward-facing camera at ~2 m height.
    """
    ...
    def project(self, pts_lidar: np.ndarray) -> np.ndarray:
        """Project N×3 LiDAR points into the image plane. Returns N×2 pixel coords (may include points behind the
        camera).
        """
...
class SensorFusion:
    ...
    Confidence fusion: weighted average of camera and LiDAR confidences, boosted by a co-detection bonus when both
    sensors agree on class.
    """

    def __init__(
        self,
        calib: CameraLiDARCalibration | None = None,
    ...
    def _lidar_to_image_bbox(self, det: LiDARDetection, box_size_px: float = 50.0) -> np.ndarray | None:
    ...
    def fuse(
        ...
        """Associate and fuse camera + LiDAR detections. Unmatched detections are kept as single-sensor outputs.
        """
        matched_cam = set()
        matched_lidar = set()
...
class TeslaPerceptionPipeline:
    ...
    Pass in pre-built detector objects to avoid circular imports: camera_detector : TeslaStyleDetector (from
    tesla_detector.py) lidar_classifier: LiDARClassifier (from pointnet_classifier.py)
    """
...
if __name__ == "__main__":
    ...
    for d in result:
        ...
        cent_str = (
            f"centroid=({d.centroid[0]:.1f},{d.centroid[1]:.1f},{d.centroid[2]:.1f})" if d.centroid is not None else ""
        )
        print(
            f"{d.source:<14} {d.class_name:<14} {d.fused_confidence:>10.3f}  "
